...
public class ClientHandler {
        ...
        new Thread(() -> {
            try {
                ...
                try {
                    ...
                    myServer.unSubscribe(this);
                    //ex.printStackTrace();
                    eventLogging.eventLog("Произошла ошибка: " + ex.getMessage(), "error");
                }
            }
        }).start();
    }
    ...
    private void authentication() throws IOException {
        while (true) {
            ...
            if (message.startsWith(AUTH_CMD_PREFIX)) {
                ...
                eventLogging.eventLog("Неудачная попытка аутентификации", "error");
            }
        }
    }
    ...
    private boolean processAuthentication(String message) throws IOException {
        ...
        if (parts.length != 3) {
            out.writeUTF(AUTHERR_CMD_PREFIX + " Ошибка аутентификации");
            eventLogging.eventLog("Ошибка аутентификации", "error");
        }
        ...
        if (username != null) {
            if (myServer.isUsernameBusy(username)) {
                ...
                eventLogging.eventLog("Логин " + username + " уже используется", "error");
                return false;
            }
            ...
            eventLogging.eventLog("Пользователь " + username + " подключился к чату", "info");
            ...
            eventLogging.eventLog("Логин " + username + " или пароль не соответствуют действительности", "error");
            return false;
        }
    }
    ...
    private void readMessage() throws IOException {
        while (true) {
            ...
            eventLogging.eventLog("message | " + username + ": " + message, "info");
